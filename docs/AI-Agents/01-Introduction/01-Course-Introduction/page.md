...
> **lightbulb** Store secrets (API keys, tokens) in a `.env` file and load them with `python-dotenv` during development. Use environment variables for CI/CD and secret managers in production.

> **warning** Never commit secrets to public repositories. Improper handling of API keys can lead to unauthorized usage and unexpected costs.
