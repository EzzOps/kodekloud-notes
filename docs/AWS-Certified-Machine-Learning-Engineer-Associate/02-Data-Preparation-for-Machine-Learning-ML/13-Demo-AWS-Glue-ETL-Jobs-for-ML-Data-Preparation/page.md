...
> **lightbulb** Make sure the IAM role provided to the Glue job has S3 read access to the source and S3 write access to the target path. If you need a starting point, attach a policy that grants `s3:GetObject`, `s3:ListBucket`, and `s3:PutObject` for the relevant prefixes. This avoids runtime failures due to permission errors.
...
> **lightbulb** After testing, remember to delete any temporary files, Glue jobs, and IAM roles you no longer need to avoid unexpected costs.

- [Watch Video](https://learn.kodekloud.com/user/courses/aws-machine-learning-associates/module/f6c821d2-a5b8-4946-9a75-624ec2ba0e75/lesson/7a8d7c73-2c93-4cfe-a0e9-3a123ddbd680)
