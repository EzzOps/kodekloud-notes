...
> **lightbulb** Stack policies refer to the resource's logical ID defined in the template (for example, `MyBucket` under `Resources`). They do not refer to the physical resource name, ARN, or bucket name.
...
> **warning** When you delete a stack, CloudFormation removes the stack's resources depending on their DeletionPolicy. If a resource is set to be deleted, removing the stack will also delete that resource (for example, an S3 bucket). Make sure you no longer need the data before deleting the stack.

- [Watch Video](https://learn.kodekloud.com/user/courses/aws-cloud-formation/module/68ab5c12-a35c-46b7-aef2-2e274c10989c/lesson/82b14b23-8cd1-405b-81de-f08c532935bf)
