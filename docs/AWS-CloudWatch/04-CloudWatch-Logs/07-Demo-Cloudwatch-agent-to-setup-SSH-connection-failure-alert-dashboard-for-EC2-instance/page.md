...
> **triangle-alert** For demonstration only: you may proceed without a key pair. **Do not** skip key pair selection in production.
...
> **lightbulb** You can add multiple `collect_list` entries to capture additional log files such as `/var/log/secure` or application logs.
