...
> **triangle-alert** Memory and disk metrics are not collected by default. Ensure the CloudWatch agent configuration file includes the appropriate sections for `mem_used_percent`, `disk_used_percent`, and any other OS-level metrics you require.

> **lightbulb** Your IAM policy must include permissions such as `cloudwatch:PutMetricData`, `logs:CreateLogStream`, `logs:PutLogEvents`, and `cloudwatch:GetDashboard` to fully leverage Resource Health.
