...
<Callout icon="lightbulb">
  If you need more predictable IOPS or higher throughput without increasing storage size, consider `gp3` (when available for your RDS engine/region) to provision IOPS and throughput independently of capacity.
</Callout>
...
<Callout icon="warning">
  Magnetic storage is legacy technology and has been phased out for most new RDS deployments. For any new instance, prefer SSD options (`gp2`/`gp3` or Provisioned IOPS) to ensure better performance and long-term support.
</Callout>
