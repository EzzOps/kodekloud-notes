...
Source: https://notes.kodekloud.com/docs/AWS-Solutions-Architect-Associate-Certification/Services-Application-Integration/SNSSQS-Demo/page

Learn to integrate Amazon SNS and SQS for automated video processing and thumbnail generation in a video application.
...
2. **SNS Topic Notification:**\
   The S3 bucket invokes an SNS notification (e.g., topic name *video-uploaded*) each time a video is uploaded.
...
2. Select the Standard queue type and name the queue *video-processing*.
...
1. Navigate to the SNS console and select the *video-uploaded* topic.
...
1. Create another Lambda function named *thumbnail-processing* using the same steps as above. Use the same role (e.g., *Lambda\_SQS\_S3*) because this function also requires SQS and S3 access.
...
1. Open the S3 console and select the raw videos bucket (e.g., *raw-videos-kodekloud*).
2. Under the "Properties" tab, scroll down to "Event notifications" and create a new notification.
3. Set an event name (e.g., *video-uploaded*). Optionally, specify a prefix (files are assumed to be uploaded to the root for this demonstration).
...
<CardGroup>
  <Card title="Watch Video" icon="video" href="https://learn.kodekloud.com/user/courses/aws-solutions-architect-associate-certification/module/2a5c60f6-2d46-4dfe-a2e2-66c7eae45a70/lesson/13f0d7fa-a75c-4d47-bca9-253ebc3c4139" />
</CardGroup>
