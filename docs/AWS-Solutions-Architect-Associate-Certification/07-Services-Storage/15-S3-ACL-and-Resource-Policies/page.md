...
> **triangle-alert** Using `"Principal": "*"` as shown above unintentionally opens access to the public, including all users from other AWS accounts and anonymous users. This can lead to excessive permissions when using `"s3:*"`, which allows all S3 operations. Always review your bucket policies carefully to avoid unintended public access.
