...
* Offline data migration workflow
  * Devices are shipped to you, you load data locally, then ship them back to AWS for ingestion into the S3 bucket(s) you specify.
  > **lightbulb** Configure the destination [S3 bucket](https://learn.kodekloud.com/user/courses/amazon-simple-storage-service-amazon-s3) and the required IAM roles/policies before ordering a Snow device. The import workflow requires a target S3 bucket.
