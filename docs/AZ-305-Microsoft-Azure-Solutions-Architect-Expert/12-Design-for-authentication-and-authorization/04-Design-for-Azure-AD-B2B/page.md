...
Source: https://notes.kodekloud.com/docs/AZ-305-Microsoft-Azure-Solutions-Architect-Expert/Design-for-authentication-and-authorization/Design-for-Azure-AD-B2B/page

This article details design considerations for implementing Azure AD B2B, focusing on integration with on-premises Active Directory and the invitation process for external users.
...
Azure AD B2B uses an invitation-based system to onboard external users. When a guest receives an invitation—sent to their email address (for example, [chris@gmail.com](mailto:chris@gmail.com))—they are guided through a sign-in process. Depending on the account configuration, the guest may need to authenticate using their Microsoft Account or Outlook password.

> **lightbulb** If the invitation process is not clearly understood, remember that the external user must complete the same secure steps as internal users to ensure compliance with your organization's access policies.
...
> **triangle-alert** Ensure that all external user access complies with your organization’s security protocols, including regular reviews of guest access and MFA policies.
