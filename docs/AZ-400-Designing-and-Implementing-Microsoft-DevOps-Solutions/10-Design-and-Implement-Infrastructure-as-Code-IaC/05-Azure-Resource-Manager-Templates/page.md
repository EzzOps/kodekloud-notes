...
> **lightbulb** Ensure you include `StartBoundary` and `RepeatDuration` when you need the schedule to span a specific time window.
...
> **triangle-alert** Avoid hardcoding secrets in your JSON. Use `[reference()]` or `[listSecrets()]` to fetch values from Key Vault at runtime.
