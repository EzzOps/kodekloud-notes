...
> **triangle-alert** Exceeding `ARG_MAX` will cause a “Argument list too long” error. For bulk operations, consider using `xargs` or reading from a file.
...
> **triangle-alert** Always use `mkdir -p` to avoid errors if the directory already exists, and add `|| exit 1` after `cd` to stop the script on failure.
...
> **lightbulb** Using `local` helps prevent variable collisions in larger scripts. For more details, see [Bash Scripting Guide](https://www.gnu.org/software/bash/manual/).
