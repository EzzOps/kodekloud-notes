...
> **lightbulb** The `|` operator connects the stdout of the left command to the stdin of the right command. Use `<` and `>` to connect files to stdin/stdout. Named pipes (FIFOs), created with `mkfifo`, are separate filesystem objects and not the same as `>`/`<` redirections.
...
> **warning** strace often requires root access (or capabilities) to trace other processes.
