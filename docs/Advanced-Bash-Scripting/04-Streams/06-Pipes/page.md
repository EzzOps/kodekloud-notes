...
Source: https://notes.kodekloud.com/docs/Advanced-Bash-Scripting/Streams/Pipes/page

This guide covers named and anonymous pipes in Bash for directing command output and enabling efficient workflows.
...
| Anonymous Pipe | \`              | \`                                                                  | Chains commands on the fly, passing stdout to stdin. |

## Named Pipes (FIFOs)

> **lightbulb** Named pipes, also known as FIFOs, are special files you create with `mkfifo`. They let processes communicate through a named file path. Here, `<` and `>` redirect standard input and output to or from file resources, sometimes referred to as file-based piping.
...
> **triangle-alert** Errors are not piped between commands. Use redirection like `2>&1` if you need to capture stderr in your pipeline.
