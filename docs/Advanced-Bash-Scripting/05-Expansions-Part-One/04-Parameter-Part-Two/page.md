...
> **lightbulb** Always enclose the expression in double quotes (`"${...}"`) to preserve spaces and prevent word splitting.
...
> **triangle-alert** Pattern matching in Bash is case-sensitive. Ensure your pattern matches the exact case of the prefix or suffix.
...
> **lightbulb** Using `${var%.txt}` leaves `.py` files untouched. For arbitrary extensions or dynamic patterns, you’ll need wildcards.
...
> **triangle-alert** Always pair `*` with a literal character (e.g., space or slash). A pattern like `${var%*}` matches the entire string, returning an empty result.
