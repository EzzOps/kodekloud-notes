...
> **lightbulb** For more advanced flag parsing, consider using `getopts` to handle short and long options.
...
> **triangle-alert** On macOS, `readlink -f` may not be available. Use `brew install coreutils` or alternative methods (`realpath`).
...
> **triangle-alert** Changing `IFS` globally can affect subsequent commands and scripts. Always restore it to its default value after making temporary adjustments.
