...
> **lightbulb** By default, `sed` reads from `stdin` or an input file and writes to `stdout`. Use the `-i` option for in-place editing.
...
> **triangle-alert** When using `-i`, changes are irreversible unless you create a backup:
