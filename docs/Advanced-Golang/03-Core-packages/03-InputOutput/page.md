...
Source: https://notes.kodekloud.com/docs/Advanced-Golang/Core-packages/InputOutput/page
...
> **lightbulb** Go uses implicit interface implementation. Unlike Java or other languages, there is no need to explicitly declare that a type implements an interface. If a type implements all methods of an interface, it is automatically considered to satisfy that interface.
