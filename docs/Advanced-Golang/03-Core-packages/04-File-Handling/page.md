...
Source: https://notes.kodekloud.com/docs/Advanced-Golang/Core-packages/File-Handling/page
...
> **lightbulb** Using `os.Stat` is a quick way to verify file attributes before processing them in your application.
...
> **lightbulb** If the file does not exist, the `os.O_CREATE` flag ensures that it is created with the specified permissions. If the file exists, the string is appended to the current content.
