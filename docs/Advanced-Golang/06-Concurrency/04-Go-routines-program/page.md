...
Source: https://notes.kodekloud.com/docs/Advanced-Golang/Concurrency/Go-routines-program/page

Learn to execute functions concurrently in Go using goroutines by prefixing function calls with the go keyword.
...
> **lightbulb** Although the elapsed time appears very short (e.g., 13 milliseconds), none of the squares are printed. This behavior occurs because the `main` function completes and exits before the goroutines have finished executing.
...
> **triangle-alert** Using `time.Sleep` is not the most reliable method to wait for goroutines to complete. A better approach is to use synchronization primitives such as [WaitGroups](https://pkg.go.dev/sync#WaitGroup), which will be covered in an upcoming lesson.
