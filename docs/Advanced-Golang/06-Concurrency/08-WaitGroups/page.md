...
Source: https://notes.kodekloud.com/docs/Advanced-Golang/Concurrency/WaitGroups/page

This article explores WaitGroups in Go for managing concurrency and synchronizing multiple goroutines effectively.
...
> **lightbulb** Ensure that every call to `wg.Add(n)` eventually corresponds to `n` calls to `wg.Done()`. Failing to do so will result in your program hanging indefinitely.
...
> **lightbulb** Using WaitGroups correctly helps to avoid potential issues with premature termination of your main routine, maintaining the reliability and predictability of concurrent execution.
