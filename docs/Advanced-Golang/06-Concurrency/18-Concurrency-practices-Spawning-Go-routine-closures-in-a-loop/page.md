...
Source: https://notes.kodekloud.com/docs/Advanced-Golang/Concurrency/Concurrency-practices-Spawning-Go-routine-closures-in-a-loop/page
...
> **lightbulb** Although this code appears straightforward, the goroutines capture the variable `i` from the outer scope. Because goroutines may start executing after the loop has advanced or finished, they can all see the same (possibly final) value of `i`.
