...
> **lightbulb** Declarative pipelines are opinionated: they give a structured syntax (`pipeline {}`, `stages`, `post`) which enables features like automatic checkout, easier visualization in Blue Ocean, and stage restart support.
...
> **warning** If your Scripted pipeline needs the repository files, remember to add an explicit checkout (e.g., `checkout scm`). Forgetting to do so will leave the workspace empty.
