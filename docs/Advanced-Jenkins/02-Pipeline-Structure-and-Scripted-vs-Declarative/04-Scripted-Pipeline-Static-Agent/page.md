...
> **lightbulb** Wrap the label in single quotes when specifying the node: `node('ubuntu-docker-jdk17-node20')`.
...
> **warning** Never hard-code secrets in your Jenkinsfile. Store credentials in the Jenkins Credentials store and reference them by credential ID (for example `mongo-credentials`). `withCredentials` masks/hides sensitive values in the console output.
