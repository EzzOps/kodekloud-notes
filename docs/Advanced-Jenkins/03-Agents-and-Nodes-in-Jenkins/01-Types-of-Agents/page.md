...
> **lightbulb** Ensure the node's remote root directory exists or is writable by the user that will run the agent. Jenkins will create subdirectories (for example, `remoting`, workspaces) under that directory.
...
> **warning** Enabling an inbound TCP port allows agents to connect to the controller. Secure this by restricting access with firewalls, VPNs, or IP allowlists and always use the agent secret. Exposing Jenkins' agent listener to untrusted networks can lead to unauthorized access.
