...
> **lightbulb** Stashes are specific to a single pipeline run. If you want to retain artifacts between runs, consider using persistent artifact storage or plugins that support preserving stashes.
...
> **warning** Stashed files are discarded when the pipeline run finishes. Do not rely on `stash` for long-term caching across separate builds—use artifact storage or a caching plugin for persistent reuse.
