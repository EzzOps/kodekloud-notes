...
> **lightbulb** When parallel stages use different agent types (Kubernetes containers vs Docker agent), they generally do not share the same filesystem or installed dependencies. Ensure each runtime has access to dependencies by running `npm install` inside that stage, or by using `stash`/`unstash` when the agent/workspace mechanisms permit it.
...
> **warning** Stashing `node_modules` can transfer platform-specific binaries that may be incompatible with the target runtime (for example, Debian/glibc binaries vs Alpine/musl). If native addons or compiled binaries are present, prefer running `npm install` in the target runtime or building artifacts in a consistent environment.
