...
> **warning** Avoid caching sensitive files or environment-specific binaries. Use the cache for reproducible dependencies (like `node_modules`) and ensure the cache validity deciding file (lockfile) is trusted.
...
> **lightbulb** Use the lockfile as the cache validity deciding file. This ensures the cache is recreated whenever dependencies change, preventing stale or incompatible node\_modules from being reused.
