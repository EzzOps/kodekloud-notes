...
> **lightbulb** Stage-level `agent` directives override the global `agent` for that stage only. If a stage is restricted to a label that no node matches, the stage will remain queued until a matching agent is available.
