...
> **lightbulb** Using `options { newContainerPerStage() }` makes Jenkins run each stage in a brand-new container created from the same Dockerfile. Files written to the container filesystem in one stage will not be visible in subsequent stages.
