...
> **lightbulb** Both styles are stored in a `Jenkinsfile` in your source repository. Declarative provides a structured, opinionated layer on top of the Pipeline DSL (Groovy), while scripted pipelines allow you to write arbitrary Groovy code for advanced scenarios.
...
> **warning** If you start with Declarative and require custom logic, extend it via `script {}` blocks or migrate parts to Scripted pipelines. Centralize shared logic in reusable libraries to avoid duplication across multiple pipelines.
