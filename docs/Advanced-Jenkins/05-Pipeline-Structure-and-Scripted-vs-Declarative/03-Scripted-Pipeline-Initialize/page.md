...
> **lightbulb** Scripted pipelines always run inside `node { ... }`. Declarative-only directives (like `options`) must be converted to `properties(...)` or equivalent build wrappers when migrating to Scripted pipelines.
