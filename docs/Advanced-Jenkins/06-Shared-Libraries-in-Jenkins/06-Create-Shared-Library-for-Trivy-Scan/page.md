...
> **lightbulb** Best practice: Pass the full image name (including tag or commit) to `vulnerability`, for example `siddharth67/solar-system:${env.GIT_COMMIT}` so scans are reproducible and traceable.

> **warning** Ensure Trivy and the HTML template file exist on the agent executing the pipeline (template path used: `/usr/local/share/trivy/templates/html.tpl`). If the template is missing or Trivy is not installed, the `reportsConverter()` and `vulnerability()` steps will fail.
