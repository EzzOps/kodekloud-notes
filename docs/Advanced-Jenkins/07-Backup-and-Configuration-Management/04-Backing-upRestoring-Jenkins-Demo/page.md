...
> **warning** Restoring configuration may require a Jenkins restart for all changes to be applied cleanly. Plan restores during maintenance windows and ensure you have a current system snapshot in case you need to roll back.
...
> **lightbulb** Enabling backup of build results and artifacts will significantly increase backup size and duration. Consider whether you need full build archives or only job configuration and metadata for recovery. If you retain build artifacts, plan for additional storage and longer backups.
