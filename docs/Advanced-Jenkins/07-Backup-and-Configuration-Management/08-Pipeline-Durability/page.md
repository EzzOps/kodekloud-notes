...
> **warning** The importer cannot convert scripted Jenkins pipelines, Jenkins credentials/secrets, or steps that depend on custom/unknown plugins. Plan to re-create secrets in GitHub and manually validate plugin-dependent steps in the resulting workflow.
...
> **lightbulb** Pipeline durability affects whether a Pipeline can recover after an unclean shutdown. Choose settings based on the criticality of the job: production deployments typically favor durability, high-throughput workloads may favor performance.
