...
> **lightbulb** Plan for plugin compatibility and integrations (e.g., secret stores, LDAP, external agents). If the target already has Jenkins installed, back it up before proceeding.
...
> **warning** Do not run two active Jenkins controllers against the same `JENKINS_HOME` or backing store simultaneously — this can cause data corruption and split-brain issues. Only one controller should own the `JENKINS_HOME`.
