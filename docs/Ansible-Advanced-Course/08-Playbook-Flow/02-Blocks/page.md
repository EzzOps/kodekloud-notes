...
> **lightbulb** Using the `when` condition and `become_user` on each task can quickly become repetitive. Blocks allow you to group tasks and assign shared directives once per group.
...
> **lightbulb** When you group tasks within a block, you can define error recovery (rescue) and actions that run regardless of success (always). This ensures a comprehensive overview of your deployment status.
