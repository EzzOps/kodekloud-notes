...
> **lightbulb** Use IP address management (IPAM) tools or Azure’s built-in features to plan non-overlapping CIDR blocks across multiple VNets.
...
> **triangle-alert** Subnets within the same VNet must not have overlapping CIDR ranges.
...
> **lightbulb** Azure NSGs include default rules permitting VNet-to-VNet traffic and outbound internet traffic. Customize NSGs to enforce your security policies.
...
> **lightbulb** When at least one `NetworkPolicy` selects a pod, all other traffic is denied by default. Be sure to explicitly allow the flows your application requires.
...
> **triangle-alert** If you choose Calico, Microsoft support engineers may not diagnose issues stemming from Calico components. For troubleshooting, refer to the [Calico documentation](https://docs.projectcalico.org/).
