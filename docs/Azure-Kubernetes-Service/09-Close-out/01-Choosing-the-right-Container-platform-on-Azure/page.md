...
> **lightbulb** Push-based pipelines are straightforward and give you direct control over each deployment step. They work well if you prefer an explicit trigger model.
...
> **lightbulb** GitOps ensures that your cluster’s live state automatically converges with the declared Git state. This model enhances auditability, reversibility, and compliance.
...
> **lightbulb** AKS supports both Linux and Windows containers via node pools. Windows containers are supported but have platform-specific constraints and fewer features compared to Linux node pools — factor this into planning and architecture decisions.

> **warning** Self-managed Kubernetes and managed OpenShift (ARO) typically increase operational overhead and may incur additional licensing or management costs. Choose them only when you need the extra flexibility they provide.
