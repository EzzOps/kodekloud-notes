...
> **lightbulb** Install `@cdktf/provider-local` so CDKTF can synthesize the corresponding Terraform `provider` block and the `local_file` resources.
...
> **warning** Using `lifecycle.ignoreChanges` prevents Terraform from reconciling the `content` property. Use it only when you intentionally want to decouple local file content management from Terraform updates.
