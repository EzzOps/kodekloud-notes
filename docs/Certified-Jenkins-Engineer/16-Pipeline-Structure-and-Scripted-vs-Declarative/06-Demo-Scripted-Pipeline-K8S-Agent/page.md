...
> **lightbulb** * In scripted pipelines, switching to a different agent/node (for example from a static agent to a Kubernetes pod via `node('label')`) does not carry over the workspace or checked-out files. Always run `checkout scm` on the node where you will execute build/test commands, or use `stash`/`unstash` to transfer files between agents.
...
> **warning** Be mindful of stash size and limits: stashing large directories (e.g., entire build artifacts) may increase build time and storage use. Prefer caching plugins or artifact repositories for large dependencies. Also ensure workspace paths and ownership are compatible across agents (uid/gid differences can affect file access).
...
- [Watch Video](https://learn.kodekloud.com/user/courses/certified-jenkins-engineer/module/956fce34-baa6-4655-a3cf-7b12d2364544/lesson/1d7d2201-c167-4e3c-951a-888cc6a54d36)
