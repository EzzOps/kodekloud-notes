...
> **lightbulb** Remember: Bash is a shell, not a persistent server process. When the Ubuntu container is launched without an attached terminal, the shell exits immediately.
...
> **lightbulb** * With CMD alone, runtime arguments replace the default command.
...
> **lightbulb** When you append an argument to the Docker run command, it overrides the default parameters defined by the CMD instruction in the Dockerfile.
...
> **lightbulb** Remember that specifying the `command` in a pod definition replaces the Dockerfile's ENTRYPOINT entirely, while the `args` field only overrides the default parameters defined by CMD.
