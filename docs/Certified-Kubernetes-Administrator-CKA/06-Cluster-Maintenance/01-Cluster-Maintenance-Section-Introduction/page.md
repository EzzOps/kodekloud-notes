...
Source: https://notes.kodekloud.com/docs/Certified-Kubernetes-Administrator-CKA/Cluster-Maintenance/Cluster-Maintenance-Section-Introduction/page
...
> **lightbulb** Before proceeding with an upgrade, always run a pre-flight check using the `kubeadm upgrade plan` command. This command helps you verify the cluster's health and fetch available upgrade versions.
...
> **triangle-alert** After upgrading the control plane with `kubeadm upgrade apply`, make sure to manually upgrade components like kubelet, as highlighted in the output above.
