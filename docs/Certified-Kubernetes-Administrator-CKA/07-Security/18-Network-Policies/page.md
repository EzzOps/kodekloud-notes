...
Source: https://notes.kodekloud.com/docs/Certified-Kubernetes-Administrator-CKA/Security/Network-Policies/page

This lesson reviews networking and security concepts, focusing on how network policies function within a Kubernetes environment.
...
> **triangle-alert** Network policies are enforced by the cluster's networking solution. While solutions like Kube-router, Calico, Romana, and Weave Net support network policies, Flannel does not enforce them. Creating policies with Flannel will not produce an error, but they won’t be applied. Always consult your network solution’s documentation to verify its support for network policies.
