...
> **triangle-alert** "cannot unmarshal number into the Go struct field Container.command of type string."
...
> **lightbulb** The recommended approach is to update your local pod manifest file and then perform a forced replace.
...
> **lightbulb** To override the ENTRYPOINT, you must use the `--command` flag or specify a new command array that includes the entire command.
