...
> **triangle-alert** Because secrets are stored as base64 encoded plaintext, anyone with access to etcd can decode and view them. Avoid storing secret definition files in public repositories without further protection.
...
> **lightbulb** Note that secrets created before enabling encryption remain unencrypted until updated. To re-encrypt these, fetch and replace them without modifying the data:
...
> **lightbulb** Direct assignment is a straightforward approach, ideal for simple scenarios or development environments.
...
> **lightbulb** Using ConfigMaps or Secrets is recommended in production environments to manage sensitive data and configuration changes without modifying the application code.
