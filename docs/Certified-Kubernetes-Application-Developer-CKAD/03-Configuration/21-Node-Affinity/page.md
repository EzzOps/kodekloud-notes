...
> **lightbulb** If your deployment requires extra flexibility—say, allowing execution on either a large or medium node—simply add both values to the `values` list.
...
> **lightbulb** "Required During Scheduling, Ignored During Execution" mandates that a Pod only starts on a node meeting the rules, whereas "Preferred During Scheduling, Ignored During Execution" allows flexibility during scheduling by placing the Pod on an alternative node if necessary.
...
> **lightbulb** The diagram illustrates the three node affinity types, highlighting that while current models ignore label changes post-scheduling, a "required during execution" model would enforce rules dynamically by evicting Pods when necessary.
