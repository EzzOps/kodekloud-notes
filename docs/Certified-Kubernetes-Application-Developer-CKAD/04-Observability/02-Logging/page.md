...
> **lightbulb** If you run the Docker container in detached mode using the `-d` option, the logs won't display on the console immediately. Instead, you can inspect the logs later by using the `docker logs` command along with the container ID.
...
> **triangle-alert** When a pod contains more than one container, running the command:
