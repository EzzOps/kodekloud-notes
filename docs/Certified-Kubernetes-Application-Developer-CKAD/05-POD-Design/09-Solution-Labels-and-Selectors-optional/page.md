...
> **lightbulb** The command above returns the number of Pods in the dev environment. In our example, the output shows there are seven Pods.
...
> **triangle-alert** The error occurs because the `matchLabels` in the selector (`tier: front-end`) do not match the label defined in the Pod template (`tier: nginx`). Ensure that the selector and the template labels are identical.
