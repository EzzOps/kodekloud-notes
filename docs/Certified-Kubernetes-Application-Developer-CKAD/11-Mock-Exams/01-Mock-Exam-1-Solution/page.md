...
> **lightbulb** Ensure your pod configuration settings (such as `containerPort`) match across all probe definitions to avoid inconsistencies.
...
> **lightbulb** For more details on configuring Ingress resources, visit [Ingress in Kubernetes](https://kubernetes.io/docs/concepts/services-networking/ingress/).
...
> **triangle-alert** Make sure you have the necessary permissions to write to `/opt/` on the control plane node.
...
> **lightbulb** Look for the container image field to identify the typo (`busyboxXXXXXXXX` instead of `busybox`).
