...
> **lightbulb** To apply this network policy, run the appropriate `kubectl create` command. Remember that network policies are enforced by the underlying network solution in your Kubernetes cluster.
...
> **triangle-alert** Even if your cluster’s network solution does not support network policies, you can define them. However, these policies will not be enforced unless supported.
