...
> **lightbulb** After updating the Ingress resource, always verify the configuration using:
...
> **triangle-alert** By default, the Ingress does not modify the URL path. If the payment application expects requests at `/` rather than `/pay`, add a rewrite annotation.
