...
> **lightbulb** The value in current-context is the name of the context in use. The context lists which cluster and which user to use — the "user" entry is a reference name, not necessarily the actual system username.
...
> **warning** The kubectl config set-credentials command modifies the kubeconfig file kubectl is currently using (typically \$HOME/.kube/config). Back up that file before making changes.
