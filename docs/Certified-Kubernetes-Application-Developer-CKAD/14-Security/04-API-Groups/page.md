...
> **lightbulb** To access the API securely, include your certificate files in the curl command as follows:
...
> **lightbulb** Do not confuse `kubectl proxy` with `kube-proxy`. The `kube-proxy` manages networking and connectivity between pods across nodes, while `kubectl proxy` forwards API requests using your kubeconfig credentials.
