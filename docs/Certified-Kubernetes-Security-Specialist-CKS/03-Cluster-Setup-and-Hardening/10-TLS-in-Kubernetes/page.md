...
> **lightbulb** Certificate files follow specific naming conventions:
...
> **lightbulb** Sometimes, servers also act as clients when communicating with other services. For example, the Kube API server communicates with the etcd server. In these scenarios, it can use its own certificate (`api-server.crt`/`api-server.key`) or a separate certificate pair specifically generated for authenticating with etcd.
