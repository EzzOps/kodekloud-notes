...
> **lightbulb** To differentiate admin users from basic users, you can include group details in the CSR by specifying the Organizational Unit (OU). For example, adding the group `system:masters` grants administrative privileges:
...
> **lightbulb** The CA root certificate is critical to verify that only valid clients can establish connections with the etcd server.
