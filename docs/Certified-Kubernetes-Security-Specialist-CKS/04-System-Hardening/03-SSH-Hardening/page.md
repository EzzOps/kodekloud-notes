...
> **lightbulb** Docker automatically applies a default Seccomp filter if your host supports Seccomp. This default filter is defined via a JSON document that whitelists approximately 60 syscalls.
...
> **triangle-alert** While blacklist profiles are easier to implement, they are inherently less secure compared to whitelist profiles due to the possibility of overlooking dangerous syscalls.
...
> **lightbulb** If you encounter connection issues, ensure that the SSH service is active on the remote server and that port 22 is open.
...
> **lightbulb** After restarting the SSH service, ensure you can log in with your SSH key. It is advised to keep an active session until you confirm that key-based authentication works properly, to avoid locking yourself out.
