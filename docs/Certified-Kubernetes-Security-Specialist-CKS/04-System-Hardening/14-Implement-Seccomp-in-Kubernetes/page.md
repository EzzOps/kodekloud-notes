...
Source: https://notes.kodekloud.com/docs/Certified-Kubernetes-Security-Specialist-CKS/System-Hardening/Implement-Seccomp-in-Kubernetes/page
...
> **lightbulb** By restricting syscalls, Seccomp helps reduce the attack surface in containerized environments. This guide outlines both default behaviors and custom configurations for running secure Kubernetes workloads.
...
> **lightbulb** By default, Kubernetes pods do not enforce Seccomp filtering; hence, fewer syscalls are blocked.
...
> **lightbulb** The `localhostProfile` path is relative to the default Seccomp profile directory (typically `/var/lib/kubelet/seccomp`). For example, if you place your custom profile in `/var/lib/kubelet/seccomp/profiles/`, the path might be `profiles/audit.json`.
...
> **triangle-alert** While a strict Seccomp profile can improve security, it may render the pod non-functional if critical syscalls are blocked.
