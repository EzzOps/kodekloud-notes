...
> **triangle-alert** Injecting faults can impact production workloads. Always run experiments in a staging environment or during scheduled maintenance windows. Monitor performance and rollback criteria closely.
...
> **lightbulb** All dependencies (Node.js, Docker, AWS CDK, `kubectl`, Helm, `eksctl`) are installed automatically via EC2 user data.
...
> **triangle-alert** If the IAM role validation fails, stop here. Ensure `fisworkshop-admin` is correctly attached before proceeding.
