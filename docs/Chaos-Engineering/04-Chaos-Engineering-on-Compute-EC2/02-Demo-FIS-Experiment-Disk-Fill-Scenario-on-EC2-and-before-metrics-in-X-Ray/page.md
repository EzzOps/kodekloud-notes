...
> **lightbulb** This SSM document does not uninstall dependencies after the test. To avoid repeated installs, pre-bake `bc` and `fallocate` and set `InstallDependencies=false`.
...
> **triangle-alert** Disk fill stress can cause system instability and data loss. Only run on non-production or isolated environments.
