...
> **lightbulb** * An existing Amazon EKS cluster with the FIS action role attached.
  * AWS CLI configured with permissions for FIS and EKS.
  * kubectl access to the EKS cluster.

> **triangle-alert** Ensure your EKS node IAM role has the `fis:StartExperiment` permission to run AWS FIS experiments successfully.
