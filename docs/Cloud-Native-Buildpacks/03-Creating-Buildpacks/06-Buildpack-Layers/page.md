...
> **lightbulb** By using build plans to transfer version metadata from the detect script to the build script, the buildpack gains flexibility and scalability. This design enables multiple teams to seamlessly specify different Node.js versions without needing to modify hardcoded values in the build script.
