...
> **lightbulb** When running build commands, environment variable changes (like modifications to PATH) only persist during the build process. At runtime, these changes do not apply. Hence, we reference the full path (`bin/node`) in the launch command.
...
> **lightbulb** This output confirms that your buildpack has been successfully packaged into a Docker image.
