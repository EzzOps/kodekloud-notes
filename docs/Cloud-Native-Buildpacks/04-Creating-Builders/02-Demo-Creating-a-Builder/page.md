...
> **lightbulb** Ensure that the IDs listed in the order section match those defined in each buildpack's own `buildpack.toml` file. You can inspect a buildpack’s metadata with the command `pack inspect-buildpack <buildpack-uri>`.
