...
> **lightbulb** This rule prevents Node 2 from accessing the web server directly, ensuring that all traffic must route through the proxy server.
...
> **lightbulb** These configuration rules ensure that general internal traffic is permitted while access from Node 2 to the web server is only allowed through the proxy server.
