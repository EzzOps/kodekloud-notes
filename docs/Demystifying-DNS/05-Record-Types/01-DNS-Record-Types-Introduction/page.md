...
> **lightbulb** In this guide, the DNS server is configured for local experimentation only, ensuring that any configuration errors do not affect live internet services.
...
> **triangle-alert** If the service status shows it as inactive (dead), make sure to start the service using the command provided below.
