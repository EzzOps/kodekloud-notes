...
Source: https://notes.kodekloud.com/docs/Demystifying-DNS/Domain-Name-Lifecycle/Negative-Caching/page

Negative caching in DNS prevents resolvers from repeatedly querying for non-existent domains, enhancing performance and reducing unnecessary load on central registries.
...
> **lightbulb** While resolvers are suggested to cache the NXDOMAIN response for 3600 seconds, they are not obligated to follow this exact duration. Some resolvers may honor the full duration, while others may cache the information for a shorter period or adjust based on their own policies.
...
> **lightbulb** Negative caching plays an important role in reducing unnecessary DNS queries and helps defend against potential DNS overload attacks by preventing resolvers from repeatedly querying for non-existent domains.
