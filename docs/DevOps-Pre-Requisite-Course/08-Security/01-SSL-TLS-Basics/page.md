...
> **lightbulb** Using the default gateway simplifies routing for internet access. Rather than adding separate routes for every external network, you set the router as the default route.
...
> **lightbulb** To check if IP forwarding is enabled, inspect the file /proc/sys/net/ipv4/ip\_forward.
...
> **lightbulb** Asymmetric encryption uses a pair of keys—a private key and a public key. When data is encrypted with the public key, only the corresponding private key can decrypt it, enhancing security over symmetric encryption.
