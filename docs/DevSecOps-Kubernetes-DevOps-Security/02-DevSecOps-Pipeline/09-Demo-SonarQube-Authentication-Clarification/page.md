...
> **triangle-alert** Using both `withSonarQubeEnv` and `-Dsonar.login` is redundant. Pick one to keep your pipeline clean and secure.
...
> **lightbulb** The `withSonarQubeEnv` step injects the authentication token automatically. No need for `-Dsonar.login`.
