...
> **lightbulb** The `install.sh` script updates packages, installs Docker CE, sets up Kubernetes tools, initializes a single-node cluster, and deploys Jenkins.
...
> **lightbulb** DAST is best suited for QA or pre-production environments. Running DAST in production can reveal critical flaws—but must be done with caution to avoid service disruptions.
...
> **triangle-alert** Running DAST against production workloads can impact performance. Schedule scans during low-traffic windows and ensure proper monitoring.
