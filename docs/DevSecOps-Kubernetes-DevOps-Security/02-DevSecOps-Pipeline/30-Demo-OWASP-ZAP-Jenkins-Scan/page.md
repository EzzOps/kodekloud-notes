...
> **lightbulb** Use **tabs** between columns—not spaces—to separate `ruleId`, `status`, and `description`.
...
> **triangle-alert** Lowering the `failBuildOnCVSS` threshold may allow medium-risk vulnerabilities to pass the build. Only do this after ensuring critical issues are remediated.
