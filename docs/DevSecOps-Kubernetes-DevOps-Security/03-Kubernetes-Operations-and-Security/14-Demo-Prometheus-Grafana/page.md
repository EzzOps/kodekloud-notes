...
> **lightbulb** You can extend `basic` or create new rule sets for network, resource limits, or Pod security policies.
...
> **lightbulb** A `NodePort` service maps a port on each node’s IP to your service, allowing external HTTP/S access without an Ingress.
