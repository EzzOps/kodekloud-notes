...
> **lightbulb** If you skip the vulnerability database update (`--skip-update`), you may miss newly disclosed CVEs.
...
> **lightbulb** Mounting `~/.cache` ensures that vulnerability databases persist between runs, dramatically reducing scan time.
...
> **lightbulb** When using KV v2, remember that paths include `/data/` (e.g., `crds/data/mysql`).
