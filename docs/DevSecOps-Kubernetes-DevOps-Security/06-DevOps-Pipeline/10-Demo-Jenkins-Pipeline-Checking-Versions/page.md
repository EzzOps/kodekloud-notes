...
> **lightbulb** The Kubernetes CLI ran locally but failed to authenticate with the API server.
...
> **triangle-alert** For production, avoid using a cluster-admin `kubeconfig`. Instead, create a ServiceAccount with minimal RBAC permissions for Jenkins.
