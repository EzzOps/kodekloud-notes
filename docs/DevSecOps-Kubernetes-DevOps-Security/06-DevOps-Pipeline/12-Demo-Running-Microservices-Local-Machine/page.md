...
> **lightbulb** Toggle the `baseURL` between `localhost` (for local debugging) and `node-service` (for Kubernetes) before running the app.
...
> **triangle-alert** Make sure port **5000** is free. If another service is using this port, stop it or choose a different host port.
