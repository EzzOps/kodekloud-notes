...
> **lightbulb** The `prepare-agent` goal instruments bytecode before tests, while the `report` goal generates HTML/XML coverage reports in `target/site/jacoco`.
...
> **triangle-alert** Any untested code (e.g., methods without assertions) will show 0% coverage. Add test cases to improve your coverage metrics.
