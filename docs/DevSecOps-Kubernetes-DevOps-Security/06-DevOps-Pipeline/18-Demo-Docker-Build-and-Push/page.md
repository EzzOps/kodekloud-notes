...
> **lightbulb** `printenv` lists all Jenkins environment variables. We leverage `$GIT_COMMIT` to tag images uniquely.
...
> **triangle-alert** Push failure means Jenkins doesn’t have Docker Hub credentials configured.
...
> **lightbulb** An empty `url` defaults to Docker Hub. Make sure `credentialsId` matches the ID you created.
