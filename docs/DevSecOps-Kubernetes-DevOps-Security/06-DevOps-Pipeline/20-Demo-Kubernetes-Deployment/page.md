...
> **lightbulb** Save this initial password—you’ll use it to generate an API token in the installer script.
...
> **triangle-alert** Ensure `jq` is installed (`sudo apt-get install -y jq`) and Jenkins is accessible at `localhost:8080` before running the script.
...
> **lightbulb** This pipeline leverages Maven for build and test, Docker for image creation and pushing, and the Kubernetes CLI (`kubectl`) for deployment.
...
> **triangle-alert** Ensure you replace the placeholder `replace` with your Docker image tag (`siddharth67/numeric-app:${GIT_COMMIT}`) before applying the manifest.
