...
> **lightbulb** The `json-file` driver is the standard Docker logging backend. It’s easy to parse and works out of the box.
...
> **triangle-alert** Modifying `daemon.json` requires restarting the Docker daemon. Existing containers will continue using their current driver until recreated.
