...
> **triangle-alert** Host ports must be unique. Attempting to bind the same host port twice will cause Docker to error out.
...
> **lightbulb** You can insert custom rules in the `DOCKER-USER` chain to filter or modify traffic before Docker’s own rules apply.
...
> **lightbulb** Use `on-failure[:<max-retries>]` to prevent infinite restart loops. Docker immediately retries with no backoff delay.
