...
> **lightbulb** Always place options (`-i`, `-t`, `-d`, `--name`) **before** the image name. Anything after the image name is interpreted as the container’s command.
...
> **lightbulb** If you specify a directory as the destination, ensure that directory already exists inside the container.
...
> **triangle-alert** Any existing file at the destination path will be overwritten without confirmation.
