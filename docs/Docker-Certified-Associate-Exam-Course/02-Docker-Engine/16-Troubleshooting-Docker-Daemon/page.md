...
> **lightbulb** `SIGTERM` is the preferred way to stop a process because it allows cleanup. `SIGKILL` should be used only if the process does not terminate gracefully.
...
> **triangle-alert** `docker container prune` permanently deletes **all** stopped containers. There is no undoing this action.
...
> **lightbulb** When using port 2376, ensure you have valid certificates configured on both client and server.
...
> **lightbulb** Adjust the `--since` flag to narrow down log entries for faster troubleshooting.
...
> **triangle-alert** A conflict between daemon flags (in `daemon.json`) and CLI or systemd overrides can prevent Docker from starting. Remove duplicate host or TLS settings.
...
> **triangle-alert** Pruning operations are irreversible. Use `docker system df` to preview reclaimable space.
