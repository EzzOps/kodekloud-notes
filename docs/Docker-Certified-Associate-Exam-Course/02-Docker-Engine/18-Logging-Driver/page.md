...
> **lightbulb** Rotating or cleaning up old JSON logs prevents disk exhaustion. Consider using Docker’s `log-opts` settings like `max-size` and `max-file`.
...
> **triangle-alert** Using the `none` driver disables all logs for the container. Only use this when you intentionally want zero log output.
