...
> **triangle-alert** Be careful when modifying `iptables` rules on production systems. Always test in a safe environment first.
...
> **lightbulb** The **bridge** network uses the `172.17.0.0/16` subnet by default, with gateway `172.17.0.1` assigned to the `docker0` interface on the host.
