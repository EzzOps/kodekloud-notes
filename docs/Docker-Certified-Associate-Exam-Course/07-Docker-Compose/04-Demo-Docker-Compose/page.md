...
> **triangle-alert** In Swarm mode, `docker stack deploy` ignores the `build` option. You must build images locally with `docker build` or push them to a registry before deploying.
...
> **lightbulb** If you want to run in detached mode, add `-d`:
