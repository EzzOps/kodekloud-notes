...
> **lightbulb** Declarative definitions allow you to scale, update, and rollback services with a single command: `docker stack deploy -c service-definition.yml my_stack`.
...
> **triangle-alert** Always test updates in a staging environment before applying to production. Use `--rollback` to revert quickly if an update misbehaves.
...
> **lightbulb** Always keep an odd number of managers (3, 5, or 7) to prevent split-brain scenarios during network partitions.
