...
> **lightbulb** Always attach your services to a user-defined overlay network for reliable DNS-based service discovery in Swarm.
...
> **lightbulb** If you omit `--replicas`, Swarm defaults to 1 replica. You can also explicitly set `--mode replicated` if you prefer.
...
> **triangle-alert** Do **not** specify `--replicas` with global services. Use only `--mode global`.
