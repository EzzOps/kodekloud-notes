...
> **lightbulb** The `selector.matchLabels` field must exactly match the labels under `template.metadata.labels`. The labels in `metadata.labels` on the ReplicaSet itself are not used for Pod selection.
