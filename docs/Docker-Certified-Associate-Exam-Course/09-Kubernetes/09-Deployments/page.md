...
> **lightbulb** Use `kubectl rollout status` to ensure a smooth update before proceeding with any dependent tasks.
...
> **triangle-alert** Recreate will interrupt service during the update. Use only when downtime is acceptable.
...
> **lightbulb** Using manifest files ensures version control, repeatability, and easier collaboration.
...
> **lightbulb** Using `kubectl apply -f` is recommended for idempotent updates. It creates or updates resources based on your manifest.
...
> **triangle-alert** If an update fails, rollback immediately:
