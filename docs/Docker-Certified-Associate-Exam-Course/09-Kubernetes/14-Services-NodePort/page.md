...
> **lightbulb** `type: LoadBalancer` only works on supported cloud environments. In unsupported setups (e.g., VirtualBox), Kubernetes falls back to assigning a NodePort without provisioning an external load balancer.
...
> **lightbulb** `NodePort` ranges from 30000 to 32767 by default. You can customize this in the API server flags.
