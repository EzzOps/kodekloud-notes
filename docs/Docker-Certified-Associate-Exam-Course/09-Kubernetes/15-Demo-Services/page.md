...
> **triangle-alert** The ReplicaSet controller enforces the desired replica count by deleting any excess Pods that match its selector.
...
> **lightbulb** NodePort Services allocate a port between `30000` and `32767`. If you omit `nodePort`, Kubernetes assigns one automatically.
...
> **triangle-alert** Exposing a Service via NodePort makes it accessible externally on the node’s network. Ensure your firewall rules and security groups are configured appropriately.
