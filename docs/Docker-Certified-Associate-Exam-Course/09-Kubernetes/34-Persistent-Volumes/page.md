...
> **lightbulb** A PVC binds to only one PV and vice versa. If a PV is larger than the requested size, the leftover space remains unused and cannot be shared.
...
> **lightbulb** If your cluster supports dynamic provisioning, you can skip creating a PV manually. Just specify a `storageClassName` in the PVC.
...
> **triangle-alert** The `hostPath` backend binds storage to a specific node’s filesystem. For highly available or multi-node clusters, use cloud volumes or networked storage solutions.
...
> **lightbulb** Adjust the backend section for other cloud providers (GCE, Azure) or network filesystems (NFS, CSI drivers) by consulting the [Kubernetes Storage Concepts](https://kubernetes.io/docs/concepts/storage/).
