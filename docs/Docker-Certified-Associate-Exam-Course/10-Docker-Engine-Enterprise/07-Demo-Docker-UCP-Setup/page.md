...
> **triangle-alert** Ensure all required network ports are open before proceeding. See [Network Ports][Network Ports].
...
> **lightbulb** Docker Engine is already installed and the host meets these specs, so we’ll skip custom volume configuration (step 3 in the guide).
