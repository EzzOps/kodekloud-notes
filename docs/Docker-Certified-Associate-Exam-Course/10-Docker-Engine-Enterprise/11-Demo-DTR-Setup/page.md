...
> **triangle-alert** Never co-locate a UCP manager and DTR on the same host—each requires its own node.
...
> **lightbulb** After removal, confirm that no Docker daemons are running:\
  `ps aux | grep docker`
