...
> **triangle-alert** While Docker is stopped, the Swarm API is unavailable. Worker containers continue running, but no changes can be made until the engine restarts.
...
> **lightbulb** If auto-locking (Swarm encryption) is enabled, the Raft database is encrypted with an unlock key stored outside `/var/lib/docker/swarm`. Securely back up this key in a password manager.
