...
> **lightbulb** The storage driver plays a crucial role in managing image layers and associated files. You can check which storage driver is active by running `docker info`.
...
> **lightbulb** The unnamed image is the one you just built because no repository name or tag was specified. To make management easier, tag your image using the `-t` parameter.
