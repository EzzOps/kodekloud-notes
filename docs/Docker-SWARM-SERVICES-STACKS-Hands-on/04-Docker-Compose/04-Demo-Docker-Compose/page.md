...
> **lightbulb** When a container is removed, its writable layer, along with all modifications, is deleted. To preserve critical data, such as database files, mount an external volume.
...
> **lightbulb** Although the `-v` flag is widely used for mounting volumes, the newer `--mount` option is preferred for its explicit syntax. For example:
...
> **lightbulb** When you deploy this updated configuration with the `docker-compose up` command, Docker automatically creates a network for the containers. All containers join the same network allowing services to refer to each other by name.
...
> **triangle-alert** Ensure that you update the database service with the required environment variables. Without `POSTGRES_PASSWORD`, dependent services may fail to establish a connection.
