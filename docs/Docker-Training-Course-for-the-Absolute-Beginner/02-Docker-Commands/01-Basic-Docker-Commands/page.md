...
> **lightbulb** The `docker ps` command is a quick way to get insights into the container statuses, while `-a` reveals a complete list including inactive ones.
...
> **lightbulb** Using `docker exec` is particularly useful for debugging or modifying container states during runtime.
