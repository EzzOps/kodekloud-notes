...
> **lightbulb** Copy and execute the join command provided on each worker node to add them to your swarm.
...
> **triangle-alert** Always execute the `docker service create` command on the manager node. Worker nodes should not run service creation commands.
