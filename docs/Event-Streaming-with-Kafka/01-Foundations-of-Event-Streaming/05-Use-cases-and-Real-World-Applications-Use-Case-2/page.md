...
> **lightbulb** Separate topics allow independent scaling, retention policies, and schema evolution. Use message keys (for example `station_id`) to ensure all messages for the same station route to the same partition (Kafka guarantees ordering within a partition) and to enable partitioned processing.
...
> **warning** For billing and financial workflows, ensure exactly-once semantics or strong deduplication. Use Kafka transactions, idempotent producers, or a robust reconciliation process to avoid duplicate charges or missing sessions.
