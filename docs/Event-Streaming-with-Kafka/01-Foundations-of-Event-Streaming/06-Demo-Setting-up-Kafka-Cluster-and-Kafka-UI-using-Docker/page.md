...
> **lightbulb** For quick exploration and ad-hoc testing the Kafka UI is very convenient. For production and automated deployments prefer the CLI, Kafka Admin APIs, or Infrastructure-as-Code tools so topic configuration is versionable and reproducible.
...
> **lightbulb** The Kafka web UI is a third‑party tool and not part of the Apache Kafka project. It provides a convenient visual way to explore cluster state (topics, brokers, consumers), but all cluster operations are still performed by Kafka itself.

> **warning** If host ports (e.g. `7000`, `9092`, `3030`) are already in use on your machine, Docker will fail to bind them. Stop conflicting services or change the published ports (host side) using `-p HOST_PORT:CONTAINER_PORT`.
