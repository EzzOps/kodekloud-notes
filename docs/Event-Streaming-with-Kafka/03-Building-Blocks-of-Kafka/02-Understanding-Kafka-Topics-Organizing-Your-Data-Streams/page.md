...
> **lightbulb** In Kafka, each partition has one leader and one or more follower replicas. Producers and consumers interact with the leader. Followers replicate the leader’s log and can be promoted to leader if the current leader fails. The replication factor controls how many copies of each partition exist.
...
> **lightbulb** Align retention settings with your application’s processing guarantees. If retention is too short, consumers might miss messages before they process them. If too long, you may incur unnecessary storage costs.
