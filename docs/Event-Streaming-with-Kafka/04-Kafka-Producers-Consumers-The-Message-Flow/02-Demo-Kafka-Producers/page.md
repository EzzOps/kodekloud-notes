...
> **lightbulb** If you omit `group_id`, Kafka treats the consumer as a new, distinct consumer; it may receive duplicates or begin from offsets determined by `auto_offset_reset`. Both `bootstrap_servers` and `topic` are required.
