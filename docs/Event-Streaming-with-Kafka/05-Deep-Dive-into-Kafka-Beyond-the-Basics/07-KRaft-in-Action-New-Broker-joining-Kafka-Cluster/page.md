...
> **lightbulb** KRaft uses the [Raft consensus algorithm](https://raft.github.io/) for controller state. Any metadata change (broker registration, topic creation, partition reassignments, config updates) must be proposed and committed to the Raft metadata log, and acknowledged by a majority of controllers, to be durable and visible cluster-wide.
