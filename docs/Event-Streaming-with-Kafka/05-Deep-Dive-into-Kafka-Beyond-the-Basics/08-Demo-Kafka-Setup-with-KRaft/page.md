...
> **lightbulb** For new Kafka deployments, prefer KRaft mode (ZooKeeper-less) because Kafka is moving towards KRaft for metadata management. Use the `config/kraft` files (`server.properties`, `controller.properties`, `broker.properties`) to tune controller and broker behavior as your cluster grows.
