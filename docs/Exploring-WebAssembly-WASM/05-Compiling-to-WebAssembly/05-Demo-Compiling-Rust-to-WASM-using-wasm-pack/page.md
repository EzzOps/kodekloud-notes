...
> **triangle-alert** Make sure you run the installer in a secure environment. The `curl | sh` pattern executes remote scripts directly, so verify the source URL before proceeding.
...
> **lightbulb** Ensure that `~/.cargo/bin` is in your `PATH`. You can add this line to your shell profile (`.bashrc`, `.zshrc`, etc.):
