...
> **lightbulb** SentenceTransformers will download model weights on first use. If you're running this in a restricted environment, pre-download models or set `SENTENCE_TRANSFORMERS_HOME` to a writable cache folder. For larger corpora consider batching embeddings to avoid memory spikes.
