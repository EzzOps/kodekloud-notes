...
> **triangle-alert** If the specified namespace (`gcp-devops-prod`) doesn't exist, the deployment will fail at the **gke-deploy** step. Ensure namespaces are created in advance.
...
> **triangle-alert** The deployment step failed because the specified GKE cluster name does not exist. Always verify that `--cluster` matches your actual cluster in the correct zone or region.
...
> **lightbulb** If you need to list clusters via the CLI, use:
