...
> **lightbulb** Use `--no-cache-dir` with `pip` to reduce image size by preventing caching of package files.
...
> **triangle-alert** Make sure sensitive files (e.g., `.env`, keys) are listed in your `.gitignore` to avoid accidental commits.
