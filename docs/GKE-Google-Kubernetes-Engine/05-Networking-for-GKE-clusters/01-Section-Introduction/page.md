...
> **lightbulb** By default, Services use the **ClusterIP** type. Change the `type` field in your Service manifest to `NodePort` or `LoadBalancer` for external access.
...
> **triangle-alert** Ensure your cluster has an Ingress controller enabled (e.g., GKE Ingress) before applying Ingress resources. Otherwise, routing rules won’t take effect.
