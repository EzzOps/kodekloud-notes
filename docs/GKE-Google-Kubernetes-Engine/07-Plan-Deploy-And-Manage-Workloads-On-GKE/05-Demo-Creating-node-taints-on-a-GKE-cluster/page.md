...
> **triangle-alert** If you remove a taint via `kubectl` on a managed node pool, GKE will not reapply it after a restart. Always define critical taints at the pool or cluster level in GKE.
...
> **lightbulb** Cluster provisioning can take **10–15 minutes**. Use `gcloud container operations list` to track progress.
...
> **triangle-alert** Removing taints allows **all** untolerated pods to schedule on this node pool. Plan accordingly.
