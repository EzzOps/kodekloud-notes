...
> **lightbulb** Only changes to `spec.template` (for example, `containers[].image` or `volumes[]`) trigger a rolling update. Scaling operations require separate commands or API calls.
...
> **triangle-alert** Using `OnDelete` means no automatic rollout. Be prepared to manually delete outdated Pods to apply your updates.
...
> **lightbulb** The `partition` setting lets you control how many Pods remain on the old revision. Adjust `maxUnavailable` or `maxSurge` to manage availability during updates.
