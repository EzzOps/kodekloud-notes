...
> **triangle-alert** If you trigger this workflow while the previous deploy is still sleeping, you’ll end up with two simultaneous deployments—often a recipe for configuration drift or resource conflicts.
...
> **lightbulb** Use a descriptive `group` name (for example, `production-deployment`) so unrelated workflows do not interfere with each other.
