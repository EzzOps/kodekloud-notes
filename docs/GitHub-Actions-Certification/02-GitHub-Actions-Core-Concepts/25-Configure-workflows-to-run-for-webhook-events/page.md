...
> **lightbulb** If you omit `required: true`, the input becomes optional and can be left blank.
...
> **triangle-alert** Ensure your `GITHUB_TOKEN` has `repo` scope; otherwise the API request will fail with a 403 error.
...
> **lightbulb** Reusable workflows accept `with` and `secrets` inputs, making them highly parameterized and secure.
