...
> **triangle-alert** Ensure your AWS credentials have `lambda:UpdateFunctionCode` and `lambda:UpdateFunctionConfiguration` permissions.
...
> **lightbulb** Setting `fail-fast: false` ensures that other matrix runs continue even if one instance fails.
