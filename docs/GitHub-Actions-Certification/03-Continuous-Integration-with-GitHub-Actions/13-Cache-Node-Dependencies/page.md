...
> **lightbulb** Use a **strong cache key** (e.g., `${{ runner.os }}-deps-${{ hashFiles('package-lock.json') }}`) to invalidate the cache automatically when dependencies change.
...
> **triangle-alert** Avoid overly broad cache paths to prevent storing unwanted files. Restrict the `path` to necessary directories only.
