...
> **triangle-alert** Never run tests against your production database. Doing so risks data corruption and performance degradation.
...
> **lightbulb** Using an isolated service container ensures your CI jobs are reproducible and safe. You can apply the same pattern for integration tests, Redis, PostgreSQL, and more.
...
> **lightbulb** Using separate containers for one-off jobs helps maintain consistent performance for core microservices.
...
> **triangle-alert** Verify your `kubeconfig` context before running `kubectl apply` to avoid unintended cluster changes.
