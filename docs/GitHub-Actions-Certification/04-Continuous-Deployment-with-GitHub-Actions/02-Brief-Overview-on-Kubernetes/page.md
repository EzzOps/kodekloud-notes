...
Source: https://notes.kodekloud.com/docs/GitHub-Actions-Certification/Continuous-Deployment-with-GitHub-Actions/Brief-Overview-on-Kubernetes/page

Kubernetes is an open-source platform for automating deployment, scaling, and management of containerized applications in cloud-native architectures.
...
> **lightbulb** By default, Pods use `restartPolicy: Always`. While containers will restart on failure, if the Pod object is deleted or its Node fails, Kubernetes will not recreate it unless managed by a higher-level controller (see Deployments).
...
> **triangle-alert** LoadBalancer Services may incur additional cloud provider costs. To consolidate routing for multiple hostnames or paths under a single IP, consider using an **Ingress** resource.
...
> **lightbulb** Ingress controllers (e.g., [NGINX](https://nginx.org/), [Traefik](https://traefik.io/)) must be installed separately to process Ingress resources. Services used by Ingress typically remain of type `ClusterIP`.
