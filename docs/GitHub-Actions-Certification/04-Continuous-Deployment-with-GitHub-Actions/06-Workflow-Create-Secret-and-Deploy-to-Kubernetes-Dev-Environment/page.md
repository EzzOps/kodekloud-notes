...
> **lightbulb** The `dev-deploy` job runs after the `docker` job (and its dependencies: `unit-testing` and `code-coverage`). It checks out the repo, replaces tokens, creates the MongoDB secret, and applies the Kubernetes manifests.
...
> **triangle-alert** If you encounter a self-signed certificate warning, safely accept it to proceed to the application.
