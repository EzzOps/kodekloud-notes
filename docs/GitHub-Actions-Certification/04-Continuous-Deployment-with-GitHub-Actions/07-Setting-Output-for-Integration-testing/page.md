...
> **triangle-alert** After this change, the `dev-deploy` job will pend if the environment has protection rules (e.g., a wait timer or approval). Administrators must review and approve to proceed.
...
> **lightbulb** Ensure your Kubernetes context and namespace (`${{ vars.NAMESPACE }}`) are correctly configured before running `kubectl`.
...
> **lightbulb** The Ubuntu runner includes both `curl` and `jq` by default. If you use a custom runner, install these tools before running the test.
