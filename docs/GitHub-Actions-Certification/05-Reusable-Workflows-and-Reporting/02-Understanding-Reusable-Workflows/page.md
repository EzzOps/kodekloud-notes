...
> **lightbulb** Make sure the `id` in the step (here: `set-ingress-host`) matches when you reference `steps.<id>.outputs`.
...
> **triangle-alert** Copy-pasting deployment steps across multiple workflows leads to fragmented updates and higher maintenance costs. A single change requires edits in every file.
...
> **lightbulb** Inputs and secrets are strongly typed. Ensure that every `workflow_call` declaration and invocation matches the expected names and types.
