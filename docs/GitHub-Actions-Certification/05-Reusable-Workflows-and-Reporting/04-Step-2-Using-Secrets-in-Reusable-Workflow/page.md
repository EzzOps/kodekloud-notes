...
> **lightbulb** See the official documentation for an overview of [reusing workflows in GitHub Actions](https://docs.github.com/actions/learn-github-actions/reusing-workflows).
...
> **triangle-alert** Always pass **all** required inputs when calling a reusable workflow:
...
> **lightbulb** Using `secrets: inherit` is quick, but explicit declaration reduces blast radius by only passing the secrets you need.
...
> **triangle-alert** If a required secret is missing, the workflow will fail to start with an “invalid workflow file” error. Always verify secret names and availability before committing.
