...
> **lightbulb** The artifact `name` in `actions/upload-artifact` must match the `name` in `actions/download-artifact`.\
...
> **triangle-alert** Using `continue-on-error: true` allows the workflow to proceed even if tests or coverage fail, but you may miss critical failures.\
