...
Source: https://notes.kodekloud.com/docs/GitHub-Actions-Certification/Custom-Actions/Create-a-Composite-Action/page

Learn to create a reusable composite GitHub Action for caching and installing NPM dependencies to improve CI/CD workflow maintainability.
...
> **lightbulb** Composite actions let you chain multiple `uses:` and `run:` steps, apply `if` conditions, and even define `pre`/`post` scripts. They don’t require separate Docker or Node environments.
...
> **triangle-alert** Composite actions currently **do not support** Docker-level isolation. All steps run in the same default environment.
