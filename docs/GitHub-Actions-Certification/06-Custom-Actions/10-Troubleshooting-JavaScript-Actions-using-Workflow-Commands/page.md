...
> **lightbulb** The `core.setSecret()` method masks sensitive values in logs. Use it whenever you log or store secrets.
...
> **triangle-alert** `core.error()` logs an error but does **not** fail the step. To stop the job on error, use `core.setFailed()`.
