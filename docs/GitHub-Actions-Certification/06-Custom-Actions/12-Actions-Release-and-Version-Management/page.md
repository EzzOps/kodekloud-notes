...
> **lightbulb** For automation, store your PAT as a GitHub Secret (e.g., `GHCR_PAT`) and reference it in workflows.
...
> **triangle-alert** By default, `GITHUB_TOKEN` only has **read** access to packages. You must explicitly set write permissions.
...
> **lightbulb** Using [Semantic Versioning] ensures clear communication of changes and consistent release management.
...
> **triangle-alert** Pinning to a branch like `main` can lead to non-deterministic builds if the branch receives breaking changes.
...
> **lightbulb** Commit SHAs are tamper-proof and cannot be moved or deleted, providing maximum stability.
