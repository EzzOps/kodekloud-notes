...
> **lightbulb** All labels under `runs-on` must exactly match those on your self-hosted runner. A mismatch (for instance, using `production` instead of `prod`) will leave the job pending until a matching runner is available or times out.
...
> **triangle-alert** Ensure your self-hosted runner has proper security controls. Exposing it directly to the internet or granting it excessive permissions can pose risks.
