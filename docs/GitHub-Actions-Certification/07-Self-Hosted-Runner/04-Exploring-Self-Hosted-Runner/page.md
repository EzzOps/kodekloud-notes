...
> **lightbulb** If you encounter connectivity or authentication issues, always start with the latest entries in `_diag/pages/Runner_*.log` and `_diag/pages/Worker_*.log`.
...
> **triangle-alert** Using `sleep 1500s` keeps the job alive long enough to explore file changes. Remember to cancel or stop the runner when done.
