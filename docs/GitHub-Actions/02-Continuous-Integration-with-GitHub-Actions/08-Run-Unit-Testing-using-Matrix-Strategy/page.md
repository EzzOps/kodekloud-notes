...
> **lightbulb** We define three Node.js versions and two OS targets. The `exclude` block omits Node.js 18 on macOS.
...
> **lightbulb** Use `${{ matrix.nodejs_version }}` and `${{ matrix.operating_system }}` to automatically pick each configured combination.
