...
> **triangle-alert** If your global coverage threshold (for example, 90%) isn’t met, the coverage step exits with a non-zero code. Subsequent steps—including artifact uploads—are skipped.
...
> **lightbulb** To ensure artifact upload even on failure, consider using `continue-on-error: true` for the coverage step or placing the upload step in a separate `always()` conditional.
