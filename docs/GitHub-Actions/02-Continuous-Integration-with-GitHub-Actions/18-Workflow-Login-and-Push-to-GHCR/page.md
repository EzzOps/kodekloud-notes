...
> **triangle-alert** If you see authentication errors, double-check that `DOCKERHUB_TOKEN` is up to date and has the correct permissions.
...
> **lightbulb** Use `vars` for non-sensitive values and `secrets` for confidential data.
...
> **triangle-alert** By default, `GITHUB_TOKEN` only has **read** access to packages. You must grant **write** permissions (shown later) to push images.
