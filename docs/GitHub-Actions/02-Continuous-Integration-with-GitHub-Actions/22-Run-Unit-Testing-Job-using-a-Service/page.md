...
> **lightbulb** Service containers are Docker containers that run in parallel with your job, providing databases, caches, or other dependencies in an isolated environment.
...
> **lightbulb** Job-level `env` entries take precedence over workflow-level `env` values.
...
> **triangle-alert** Check for typos and ensure `mongodb://` (not `mongodb+srv://`) when connecting to a local container.
