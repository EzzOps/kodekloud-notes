...
> **lightbulb** By specifying `container.image`, all subsequent steps run inside that Docker image. You no longer need a separate setup action for Node.js.
...
> **triangle-alert** Make sure the service container name (`mongo`) matches the hostname used in `MONGO_URI`. Incorrect naming can lead to connection failures.
