...
> **lightbulb** Only HTTP/S URLs are supported in the `environment.url` field.
...
> **lightbulb** The `curl` and `jq` utilities are pre-installed on the `ubuntu-latest` runner.

> **triangle-alert** Using `curl -k` skips TLS verification. Only use this in non-production or trusted test environments.
