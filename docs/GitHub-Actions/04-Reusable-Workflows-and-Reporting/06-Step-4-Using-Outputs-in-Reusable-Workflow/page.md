...
> **triangle-alert** If you don’t define `outputs` in `workflow_call`, any values you set inside the workflow won’t be available to the caller.
...
> **lightbulb** We use `${{ jobs.reuse-deploy.outputs.APP_INGRESS_URL }}` to map the job’s output to the workflow’s `application-url`.
