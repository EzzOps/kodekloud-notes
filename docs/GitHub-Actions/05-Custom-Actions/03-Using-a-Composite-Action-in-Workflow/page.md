...
> **lightbulb** You can omit the `action.yml` filename and simply point to the directory containing your composite action.
...
> **triangle-alert** Every `run` step in a composite action must specify a `shell` (e.g., `bash`, `pwsh`, `sh`, `cmd`) depending on the runner OS.
