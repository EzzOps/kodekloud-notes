...
> **lightbulb** Workflows must reside in `.github/workflows`; files ending with `.yml` or `.yaml` will be automatically detected.
...
> **triangle-alert** By default, each job runs in a clean VM/container. Use [`needs`](https://docs.github.com/actions/using-jobs/defining-job-dependencies) to share artifacts or enforce order.
