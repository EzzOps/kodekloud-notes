...
> **triangle-alert** If you omit `actions/checkout`, subsequent steps won’t have access to your code, leading to errors such as `file not found` when running commands like `ls` or `cat`.
...
> **lightbulb** Run `ls -a` to display all files, including those starting with a dot.
