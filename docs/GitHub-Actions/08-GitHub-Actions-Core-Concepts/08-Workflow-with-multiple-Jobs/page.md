...
> **lightbulb** By default, each job executes on a clean runner and does not share files. Use [artifact actions](https://docs.github.com/actions/publishing-packages/publishing-build-artifacts) or define dependencies to pass data between jobs.
...
> **triangle-alert** Each job runs in isolation on its own VM. Without artifacts or explicit job dependencies, files like `dragon.txt` won’t persist across jobs.
...
> **lightbulb** Since `test_job_2` may start before `build_job_1` finishes, any attempt to read `dragon.txt` will trigger a file-not-found error.
