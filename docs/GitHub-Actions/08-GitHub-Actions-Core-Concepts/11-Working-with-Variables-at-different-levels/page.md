...
Source: https://notes.kodekloud.com/docs/GitHub-Actions/GitHub-Actions-Core-Concepts/Working-with-Variables-at-different-levels/page

Learn to declare environment variables at different levels in GitHub Actions to streamline workflows and secure sensitive data.
...
> **triangle-alert** Never hard-code sensitive values like passwords or API keys. Store them in [GitHub Secrets](https://docs.github.com/actions/security-guides/encrypted-secrets) and reference them with `${{ secrets.YOUR_SECRET_NAME }}`.
...
> **lightbulb** Use `${{ env.VAR_NAME }}` when combining expressions with literal strings to ensure consistent parsing.
