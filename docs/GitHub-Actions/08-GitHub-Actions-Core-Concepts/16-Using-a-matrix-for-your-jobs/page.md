...
> **lightbulb** Matrix strategy automatically runs all defined combinations in parallel, speeding up CI/CD feedback.
...
> **triangle-alert** A single matrix failure by default stops all running jobs. Use `strategy.fail-fast: false` or exclude incompatible combinations with `matrix.exclude`.
