...
> **lightbulb** A few MDX-specific tips for editing READMEs rendered as MDX:
...
> **warning** When adding examples with placeholders (e.g., `<PORT>`, `<name>`) or template variables (e.g., `{{inputs.param}}`), always wrap them in backticks to prevent the MDX parser from interpreting them as JSX or template syntax.
