...
* Templates: Repository templates or starter templates bootstrap a new project. Templates commonly include a `.devcontainer` folder (with `devcontainer.json` and an optional Dockerfile) that specifies the container image, required packages, extensions, and forwarded ports.
  > **lightbulb** Using a `.devcontainer` codifies the OS image, packages, and editor extensions so every codespace launched from the repo delivers a consistent developer experience.
...
> **warning** Codespaces availability and billing depend on your GitHub account or organization plan and policies. Verify enabled regions, allowed machine types, and usage quotas in your organization settings before provisioning large or long-running environments.
...
- [Watch Video](https://learn.kodekloud.com/user/courses/github-foundation-certification/module/c4995815-313c-40eb-a9c1-aedee41abd7d/lesson/9f74e8f1-7b27-4a89-8552-30978af919d8)
...
> **lightbulb** Set automatic stop timeouts for idle Codespaces to reduce costs. Organizations and users can configure stop timeouts so idle Codespaces are automatically stopped after a configured period.
