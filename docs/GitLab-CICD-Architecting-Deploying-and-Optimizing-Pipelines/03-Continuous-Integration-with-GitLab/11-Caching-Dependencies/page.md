...
- [Watch Video](https://learn.kodekloud.com/user/courses/gitlab-ci-cd-architecting-deploying-and-optimizing-pipelines/module/3a1c2306-8091-4dfe-b40f-e2ca53918553/lesson/20c65bbd-2d75-42be-b0cc-02bb346d6512)
...
![The image shows a GitLab documentation page about the cache:policy keyword in CI/CD YAML syntax, explaining how to configure cache upload and download behavior with possible inputs like pull, push, and pull-push.](https://kodekloud.com/kk-media/image/upload/v1752877240/notes-assets/images/GitLab-CICD-Architecting-Deploying-and-Optimizing-Pipelines-Caching-Dependencies/gitlab-cache-policy-cicd-yaml.jpg)
...
> **lightbulb** Using `package-lock.json` in the cache key ensures the cache is invalidated automatically whenever your dependencies change.

## Viewing the Pipeline

After committing `.gitlab-ci.yml`, GitLab triggers a pipeline. The **Pipelines** page shows status and stages:

![The image shows a GitLab CI/CD pipeline interface with various pipeline statuses such as "Running," "Skipped," and "Failed." It includes details like pipeline IDs, branches, and user avatars.](https://kodekloud.com/kk-media/image/upload/v1752877241/notes-assets/images/GitLab-CICD-Architecting-Deploying-and-Optimizing-Pipelines-Caching-Dependencies/gitlab-ci-cd-pipeline-statuses.jpg)

Within the project view you’ll see jobs like `unit_testing` and `code_coverage`:

![The image shows a GitLab CI/CD pipeline interface for a project named "Solar System NodeJS Pipeline," displaying the status of jobs like "code\_coverage" and "unit\_testing." The sidebar includes options for managing the project, such as issues, merge requests, and pipelines.](https://kodekloud.com/kk-media/image/upload/v1752877242/notes-assets/images/GitLab-CICD-Architecting-Deploying-and-Optimizing-Pipelines-Caching-Dependencies/gitlab-ci-cd-solar-system-pipeline.jpg)

### First Run: Cache Miss

On the initial run, no cache exists. The pipeline installs dependencies and then uploads the cache:

![The image shows a GitLab CI/CD pipeline job interface with a successful unit testing job. The console output details the steps executed, and the job status is marked as "passed."](https://kodekloud.com/kk-media/image/upload/v1752877243/notes-assets/images/GitLab-CICD-Architecting-Deploying-and-Optimizing-Pipelines-Caching-Dependencies/gitlab-ci-cd-pipeline-success.jpg)
...
![The image shows a GitLab CI/CD pipeline job interface with a successful unit testing job. The console output displays steps like restoring cache, executing scripts, and uploading artifacts.](https://kodekloud.com/kk-media/image/upload/v1752877245/notes-assets/images/GitLab-CICD-Architecting-Deploying-and-Optimizing-Pipelines-Caching-Dependencies/gitlab-ci-cd-pipeline-success-2.jpg)
...
> **triangle-alert** Clearing caches too frequently may negate performance gains. Only clear when dependencies are truly out of sync.
...
- [Watch Video](https://learn.kodekloud.com/user/courses/gitlab-ci-cd-architecting-deploying-and-optimizing-pipelines/module/3a1c2306-8091-4dfe-b40f-e2ca53918553/lesson/efb48dc1-13a6-4929-932d-083b91931ab2)
