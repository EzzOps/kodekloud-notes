...
> **lightbulb** The triangle shape is colored white, which makes it invisible on a white background.
...
> **lightbulb** The Lua script checks if the `TRIANGLE_COLOR` value in the ConfigMap is set to "white". If so, it marks the resource as "Degraded" and returns a corrective message.
