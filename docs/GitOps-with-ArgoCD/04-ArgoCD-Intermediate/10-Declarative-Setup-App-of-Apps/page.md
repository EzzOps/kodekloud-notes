...
> **lightbulb** The multi-app configuration retrieves manifests from a specific directory, providing a centralized configuration for managing nested applications.
...
> **lightbulb** Ensure that no old configurations interfere with the new deployment by removing outdated applications before applying the new manifest.
...
> **lightbulb** Many users advocate that Pluto should not be classified as a planet due to its distant orbit and retrograde rotation, which is why an application variant excluding Pluto is provided.
