...
Source: https://notes.kodekloud.com/docs/GitOps-with-FluxCD/Flux-Overview/FluxCD-Installation-Options/page

Explore various installation methods for FluxCD and GitOps on Kubernetes, including prerequisites, CLI setup, bootstrapping, customization, and uninstallation.
...
> **triangle-alert** Using End-of-Life (EOL) Kubernetes versions in production is **not recommended**. Upgrade your cluster if you’re on a version older than v1.20.6.
...
| ---------------- | --------------------------------------------------- | ------------------------------------- |
| Linux            | `curl -s https://fluxcd.io/install.sh \| sudo bash` | [FluxCD Install Script][flux-install] |
| macOS            | `brew install fluxcd/tap/flux`                      | [Homebrew FluxCD Tap][flux-homebrew]  |
| Windows          | `scoop install fluxcd` or `choco install fluxcd`    | [FluxCD Releases][flux-releases]      |

> **lightbulb** Running the install script (`install.sh`) automatically places the `flux` binary in your `$PATH`.
...
> **lightbulb** The `bootstrap` command is idempotent—running it multiple times will not change an already-bootstraped setup.
...
| ------------------------- | ------------------------------------------------------------------------------------------------------------------- | ---------------------------------- |
| Terraform                 | Bootstrap FluxCD declaratively using Terraform and the Flux provider                                                | [Flux Terraform Provider][tf-flux] |
...
> **triangle-alert** `flux uninstall` **does not** delete Kubernetes objects or Helm releases that Flux managed—those resources remain in your cluster.
...
[tf-flux]: https://registry.terraform.io/providers/fluxcd/flux/latest
