...
> **triangle-alert** The `--prune=true` flag will remove any Kubernetes resources in the target namespace that are not tracked by this Kustomization. Ensure you don’t have unmanaged resources in `2-demo`.
...
> **lightbulb** Ensure you’ve bootstrapped Flux CD with SSH credentials so the `secretRef` in your `GitRepository` can authenticate to GitHub.\
