...
> **triangle-alert** Once set, constant values in Go are immutable. Modifying a constant will result in a compile-time error.
...
> **triangle-alert** Remember: Constants in Go must be assigned a value at the time of declaration, and the shorthand `:=` is not permitted.
