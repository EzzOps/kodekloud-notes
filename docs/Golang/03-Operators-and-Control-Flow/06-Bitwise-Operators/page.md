...
> **lightbulb** Note: In Go, ^ is also the unary bitwise NOT (complement) when used with a single operand (for example `^x` flips all bits of `x`). In the examples above `^` is used as a binary XOR operator.
