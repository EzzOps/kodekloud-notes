...
> **lightbulb** The default zero value for integers is 0. When working with arrays, remember that the array's capacity is equal to its length. However, for slices, the capacity begins from the first element accessed in the underlying array.
