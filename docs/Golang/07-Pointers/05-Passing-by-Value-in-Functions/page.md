...
> **lightbulb** When a variable is passed by value, its original location in memory remains unchanged even if the function alters the copied parameter.
...
> **lightbulb** Passing by value means that a copy of the variable is provided to the function. Any changes to the parameter inside the function do not alter the original variable outside the function.
