...
> **lightbulb** You can add additional labels under the `labels` section (for example, `env: production`) to help refine log queries in Grafana.
...
> **lightbulb** Ensure the startup logs display no errors. Look for messages indicating that Promtail is reloading the configuration and listening on the designated HTTP and gRPC ports.
