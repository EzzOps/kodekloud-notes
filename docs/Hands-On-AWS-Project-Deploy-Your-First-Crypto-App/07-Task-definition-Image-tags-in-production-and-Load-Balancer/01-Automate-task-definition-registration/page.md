...
> **lightbulb** Store your ECS task definition in source control so its changes are auditable and reproducible. Use the CI pipeline to register new revisions automatically.
...
> **warning** Ensure the CodeBuild service role has permissions to call `ecs:RegisterTaskDefinition` and `ecr:GetAuthorizationToken` / `ecr:BatchCheckLayerAvailability` / `ecr:GetDownloadUrlForLayer` / `ecr:PutImage` as needed. Missing IAM permissions will cause the build to fail.
