...
> **lightbulb** Read replicas always reflect the latest cluster state but cannot process `acl replication up` or write-related API calls.
...
> **triangle-alert** Ensure your network allows bidirectional communication between primary servers and read replicas. Replication requires stable connectivity to avoid replication lag.
