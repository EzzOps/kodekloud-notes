...
> **lightbulb** Environment variables are useful for overrides in containerized or CI/CD environments. For complex configurations, maintain a single JSON or HCL file.
...
> **triangle-alert** Keep your gossip encryption key (`encrypt`) confidential. If exposed, rotate it immediately using `consul keygen` and reconfigure all agents.
...
> **lightbulb** You can define multiple health checks per service. Use `deregister_critical_service_after` to automatically remove unhealthy instances.
