...
> **lightbulb** Only the `name` field is required. If you omit `id`, Consul uses the value of `name`. Explicitly setting `id` helps avoid collisions when running multiple instances.
...
> **triangle-alert** If health checks continually fail, the service remains in the catalog but marked unhealthy—it will not receive traffic until the check passes.
