...
> **lightbulb** The `service.json` file follows Consul’s service definition schema. You can extend it later with health checks under the `"Check"` block.
...
> **triangle-alert** Without a defined health check, Consul cannot detect application failures. Always configure checks to get accurate health status!
...
> **lightbulb** You can also deregister services via the Consul UI or the HTTP API by specifying the service ID.
