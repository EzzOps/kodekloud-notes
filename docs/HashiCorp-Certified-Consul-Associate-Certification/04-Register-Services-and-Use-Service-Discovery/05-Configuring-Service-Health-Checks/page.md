...
> **triangle-alert** A failing host-level check removes **all** services on that node from Consul queries, even if their individual checks pass. Application-level failures only impact the specific service.
