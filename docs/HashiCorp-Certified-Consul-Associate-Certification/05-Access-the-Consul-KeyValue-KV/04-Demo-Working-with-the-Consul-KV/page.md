...
> **lightbulb** Make sure to update the version number in the URL if a newer release is available.
...
> **triangle-alert** If your app requires a Consul ACL token, set `CONSUL_HTTP_TOKEN` in the environment or pass `-token` to `envconsul`.
