...
> **lightbulb** By default, Consul Template polls Consul every two seconds. Adjust the interval with the `-wait` flag or in HCL.
...
> **triangle-alert** Ensure the Consul agent is running and reachable. When ACLs are enabled, include the `token` parameter in your HCL or CLI invocation.
