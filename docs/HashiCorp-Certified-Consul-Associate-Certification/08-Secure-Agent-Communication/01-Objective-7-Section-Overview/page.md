...
> **lightbulb** Consul will verify peer hostnames on incoming connections. Ensure your certificate’s Common Name (CN) or SAN matches the agent’s advertised address.
...
> **lightbulb** When integrating with Vault, configure ACL tokens and policies to allow Consul to request and renew certificates automatically.
...
> **lightbulb** Consul uses mutual TLS (mTLS) to enforce identity verification and data confidentiality across all RPC calls.
...
> **triangle-alert** Protect your private keys (`.pem` files). Unauthorized access may allow attackers to impersonate nodes.
