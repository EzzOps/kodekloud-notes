...
> **lightbulb** You can also supply rules inline using `--rules 'service "eCommerce" { policy = "write" }'` if you prefer not to use a file.
...
> **lightbulb** If your default ACL policy is `allow`, you can omit the `X-Consul-Token` header.
...
> **triangle-alert** Grant only minimal privileges to the anonymous token to avoid exposing sensitive data.
...
> **triangle-alert** Keep the bootstrap token offline or in a secure vault. It has unrestricted `global-management` rights and should only be used to create scoped tokens.
...
> **lightbulb** Clients must use the **SecretID** when making API calls. Supplying the AccessorID instead will result in a “permission denied” error.
