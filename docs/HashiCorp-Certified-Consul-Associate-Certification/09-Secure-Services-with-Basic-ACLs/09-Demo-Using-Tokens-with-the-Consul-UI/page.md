...
> **lightbulb** Using `CONSUL_HTTP_TOKEN` is convenient for CI/CD pipelines and local development shells.
...
> **lightbulb** Ensure the token file has restrictive permissions (`chmod 600 token.txt`) to prevent unauthorized access.
...
> **lightbulb** * A running Consul cluster with ACL enforcement enabled
...
> **triangle-alert** Treat your bootstrap token like a root credential. Avoid sharing it or embedding it in scripts. Always store tokens in a secure vault or use a short-lived token for day-to-day operations.
