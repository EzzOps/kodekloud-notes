...
> **triangle-alert** Do **not** remove the old key until all nodes report the new key as primary. Premature removal can lead to cluster partitions and service disruptions.
...
> **lightbulb** When federating multiple datacenters (WAN gossip), use the same encryption key in each datacenter. This ensures messages can be decrypted and forwarded properly across the WAN pool.
...
> **lightbulb** Treat your gossip key like a password. Store it in a secure vault or environment variable—never commit it to version control.
...
> **triangle-alert** Ensure all agents have pulled the new key before removing the old one. Nodes missing the primary key will be unable to decrypt gossip traffic and may leave the cluster.
