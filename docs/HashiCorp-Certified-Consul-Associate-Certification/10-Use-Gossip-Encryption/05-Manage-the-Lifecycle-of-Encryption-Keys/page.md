...
> **lightbulb** Consul’s gossip encryption uses a 32-byte Base64 key. You can generate this key with any tool, but `consul keygen` guarantees compatibility.
...
> **triangle-alert** Avoid running with multiple active keys longer than necessary. Each Consul agent will attempt decryption with every key on inbound messages, increasing CPU overhead.
