...
> **triangle-alert** If an intention denies communication, the mTLS handshake will fail and Envoy will reject the connection, enforcing your security policies.
...
> **lightbulb** Consul is GA and available at a low hourly rate for development environments. Vault remains in beta.
...
> **lightbulb** Refer to the [AWS VPC Peering documentation](https://docs.aws.amazon.com/vpc/latest/peering/what-is-vpc-peering.html) when updating route tables.
...
> **triangle-alert** If you encounter errors about invalid ACL keys, ensure `tokens` (not `token`) is under the `"acl"` section and that boolean keys (`encrypt_verify_incoming`, etc.) match the HCP-generated config.
