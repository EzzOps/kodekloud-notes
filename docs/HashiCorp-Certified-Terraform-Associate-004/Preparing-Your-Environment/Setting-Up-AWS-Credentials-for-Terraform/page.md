...
| ---------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------: | ------------------------------------------------------------------------------------------------------------------ |
| macOS / Linux (bash, zsh) — session-only |       `bash export AWS_ACCESS_KEY_ID="[AWS_ACCESS_KEY_ID]" export AWS_SECRET_ACCESS_KEY="examplesecretKEY+chars" export AWS_DEFAULT_REGION="us-east-1" ` | Exports apply only to the current shell session. Persist in `~/.bashrc` or `~/.zshrc` if needed (beware security). |
| Windows PowerShell — session-only        | `powershell $Env:AWS_ACCESS_KEY_ID = "[AWS_ACCESS_KEY_ID]" $Env:AWS_SECRET_ACCESS_KEY = "examplesecretKEY+chars" $Env:AWS_DEFAULT_REGION = "us-east-1" ` | Session-only — values disappear when the shell closes.                                                             |
