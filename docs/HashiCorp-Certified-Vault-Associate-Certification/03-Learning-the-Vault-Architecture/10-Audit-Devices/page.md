...
> **lightbulb** Ensure the operating-system user running Vault (commonly `vault`) has write permissions for `/var/log/vault_audit_log.log`. Without proper permissions, Vault will fail to enable the audit device.
...
> **triangle-alert** Vault prioritizes security over availability: if it cannot write to any enabled audit device, it will refuse client requests and effectively go offline rather than risk losing audit data.
