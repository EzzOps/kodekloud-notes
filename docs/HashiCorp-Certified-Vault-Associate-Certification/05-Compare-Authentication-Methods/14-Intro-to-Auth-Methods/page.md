...
> **lightbulb** Auth Methods enable you to integrate Vault with various identity providers—human (username/password, Okta, GitHub) or machine-to-machine (AWS, Kubernetes). Regardless of the method, the goal is to obtain a Vault token.
...
> **triangle-alert** Always include `X-Vault-Token` in your HTTP headers or use the `VAULT_TOKEN` environment variable. Otherwise, Vault will not prompt for credentials and will deny access.
