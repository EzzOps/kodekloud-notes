...
> **triangle-alert** If a single operator runs `vault operator init` without encryption, they receive all keys in plaintext. Always encrypt shards when splitting custody.
...
> **lightbulb** These flags adjust only the share count and threshold. They do **not** encrypt the output.
