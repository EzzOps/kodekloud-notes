...
> **lightbulb** `terraform login` opens a browser for authentication and saves your token in `~/.terraform.d/credentials.tfrc.json`.
...
> **triangle-alert** Keep your `TOKEN` secure. Do not commit it to source control or share it publicly.
...
> **lightbulb** On macOS, use `sed -i ''` for inline replacements.
