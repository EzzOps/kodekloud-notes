...
Source: https://notes.kodekloud.com/docs/Helm-for-Beginners/Helm-Charts-Anatomy/Chart-Hooks/page

This article explores how Helm chart hooks enhance Kubernetes deployments by automating actions before and after upgrades.
...
> **lightbulb** To designate a resource as a hook, add the annotation key `helm.sh/hook` with a value such as `pre-upgrade`. This tells Helm to execute the Job as part of the pre-upgrade process rather than a regular installation.
...
> **triangle-alert** If the hook fails, the Job resource will not be deleted automatically. This can be useful for debugging, but be mindful of lingering resources that might require manual cleanup.
