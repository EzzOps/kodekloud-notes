...
> **lightbulb** Both Replication Controllers and ReplicaSets serve the same fundamental purpose: they ensure that your desired number of pod replicas are running. However, the ReplicaSet, which is part of the `apps/v1` API, introduces improvements such as the mandatory selector field.
