...
> **warning** Ports below 1024 are privileged. To bind host port 80 to a container, run the command with elevated privileges (sudo) or map a non-privileged host port (>=1024).
...
> **lightbulb** Ensure Intel VT-x or AMD-V is enabled in your BIOS/UEFI before deploying fully virtualized guests.
