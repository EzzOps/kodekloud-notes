...
Source: https://notes.kodekloud.com/docs/Microsoft-Azure-Security-Technologies-AZ-500/Key-Vault/Configure-key-vault-access/page

This article explains how to securely access data stored in Azure Key Vault using Azure AD for authentication and RBAC or Access Policies for authorization.
...
The Function App retrieves an access token from Azure AD for "[https://vault.azure.net](https://vault.azure.net)", then calls the Key Vault API to get the secret.
...
In this block, the script retrieves the tenant ID, client ID, and client secret from environment variables, while the Key Vault name and secret name come from URL parameters. It then calls Azure AD to acquire an access token for "[https://vault.azure.net](https://vault.azure.net)" and prepares an authorization header for subsequent API requests.
