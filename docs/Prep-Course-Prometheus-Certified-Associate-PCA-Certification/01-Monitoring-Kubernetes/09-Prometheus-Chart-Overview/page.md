...
> **lightbulb** The release name you provide when installing a chart (for example `prometheus` in this guide) is arbitrary. Choose any valid name; just be consistent when running `helm upgrade`, `helm uninstall`, or when querying resources by label.
...
> **lightbulb** To deploy with your custom values file, pass it during install or upgrade: `helm install prometheus prometheus-community/kube-prometheus-stack -f values.yaml` (or `helm upgrade --install prometheus ... -f values.yaml`).
...
> **lightbulb** The Helm chart also installs Grafana and configures it to use the Prometheus datasource out of the box. Grafana provides a graphical UI to visualize Prometheus metrics (installed automatically by the chart).
...
> **warning** Do not manually edit the generated Secret or ConfigMap created by the operator. Changes will be overridden by the operator. Instead, use Kubernetes-native objects supported by the Prometheus Operator (ServiceMonitor, PodMonitor, PrometheusRule, Prometheus CR) to configure scraping and rules.
