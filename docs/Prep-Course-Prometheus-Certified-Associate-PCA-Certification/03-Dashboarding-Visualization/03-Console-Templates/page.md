...
> **lightbulb** Use labels to keep metrics organized and queryable. However, be cautious about label cardinality: labels with high cardinality (for example, user IDs, session IDs, or other highly unique values) can produce a huge number of time series and cause performance and storage problems. Restrict labels to a small, well-defined set of values.
...
> **lightbulb** Console templates are regular HTML files using Go templates. Reuse provided fragments like `head`, `prom_content_head`, and `tail` for consistent styling and JS helpers. You can embed PromQL expressions and use built-in widgets for drilldowns and graphs.
