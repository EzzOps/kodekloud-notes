...
> **lightbulb** It's good practice to run exporters as a non-privileged, system user (no shell and no home). This reduces the blast radius if the exporter is compromised.
...
> **lightbulb** Download the Node Exporter binary that matches your OS/architecture. Optionally verify the SHA256 checksum listed on the release page with `sha256sum` before extracting the archive.
