...
> **lightbulb** Use the Expression Browser when you need fast, one‑off queries or to verify metric names/labels. For structured dashboards and alerts, use a dashboarding solution such as Grafana.
...
> **warning** The Graph tab expects an instant vector to plot. Supplying a bare range vector will cause an error — wrap the range vector in an aggregation/function (for example `rate()`), or remove the range selector for plotting an instant vector.
