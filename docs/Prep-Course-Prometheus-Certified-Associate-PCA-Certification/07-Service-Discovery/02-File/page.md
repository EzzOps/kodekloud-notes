...
> **lightbulb** Prometheus watches the configured files and automatically reloads targets when they change. Use globs to split targets across files (per-team or per-environment) and keep your `prometheus.yml` small and consistent.
...
> **warning** Treat file-based configuration as sensitive: files may contain labels or metadata you don’t want exposed. Secure file permissions and avoid storing credentials in these files.
