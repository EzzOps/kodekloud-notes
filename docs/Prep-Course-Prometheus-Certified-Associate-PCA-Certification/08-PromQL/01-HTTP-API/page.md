...
> **lightbulb** Refer to the Prometheus documentation under [Querying → Functions](https://prometheus.io/docs/prometheus/latest/querying/functions/) for a complete list of functions and their exact semantics.
...
> **lightbulb** The `/api/v1/query` endpoint performs an instant query (value at a single point in time). For multi-point time series over a time range, use `/api/v1/query_range`.

> **warning** When using `curl --data` on the shell, wrap the entire value in single quotes and use double quotes inside PromQL label selectors to avoid quoting conflicts. Example: `--data 'query=node_cpu_seconds_total{job="node"}'`.
