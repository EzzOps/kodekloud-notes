...
> **lightbulb** Use subqueries when you need to take an instant-expression (for example, `rate(...)`) and evaluate that expression across a historical time window at a given resolution so that range-based aggregation functions (like `max_over_time`, `min_over_time`, etc.) can operate on a range vector.
