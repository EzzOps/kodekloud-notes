...
> **warning** Be careful with `chown -R` — do NOT run `sudo chown -R alertmanager:alertmanager /etc/` or similarly broad commands. Target only `/etc/alertmanager` (and other specific directories) to avoid damaging system ownerships.
