...
> **lightbulb** When you want to replace all metrics for a job, use `push_to_gateway` (PUT semantics). Use `pushadd_to_gateway` to add/merge metrics (POST semantics). To remove metrics for a job or grouping, use `delete_from_gateway`.
...
> **warning** Ensure the Prometheus Pushgateway is reachable from your process. Choose `job` and any additional grouping labels carefully to avoid unintentionally overwriting or deleting other metrics.
