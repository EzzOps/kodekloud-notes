...
> **lightbulb** Rule `labels` are critical: they enable Alertmanager to match, group, and route alerts. Use `annotations` for human-readable alert text and details.
...
> **lightbulb** Grouping by labels such as `team` and `env` reduces notification noise by bundling related alerts into one message per label combination.
