...
Source: https://notes.kodekloud.com/docs/Python-API-Development-with-FastAPI/Working-with-Databases/Inserting-Data/page

This lesson explores how to insert new rows into a PostgreSQL database using SQL queries.
...
When constructing an INSERT statement, you must specify the target table and the columns that need to be populated. In our example, the "products" table requires values for the "name" and "price" columns. Optional columns such as "is\_sale" and "inventory" automatically default to false and zero, respectively, if not provided.
...
Below is an example that inserts values into the "name", "price", and "inventory" columns. By omitting the "is\_sale" column, PostgreSQL will use its default value (false):
