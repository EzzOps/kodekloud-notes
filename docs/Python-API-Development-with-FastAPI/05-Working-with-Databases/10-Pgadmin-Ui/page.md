...
Source: https://notes.kodekloud.com/docs/Python-API-Development-with-FastAPI/Working-with-Databases/Pgadmin-Ui/page

This article explores PgAdmin, a GUI for managing PostgreSQL, covering server setup, database creation, table design, and data manipulation.
...
### Adding a Boolean Column ("is\_sale")
...
2. Add a new column named "created\_at" and set its data type to "timestamp with time zone" to capture both the date and time.
3. Set the default value to `now()` so PostgreSQL automatically records the creation time upon inserting a new record.
4. Save your changes. Note that for existing rows, PostgreSQL will update the "created\_at" field with the current timestamp.

To verify, insert a new product (for instance, a "keyboard" with a price of `28` and an inventory of `50`), and the "created\_at" field will be populated automatically:
