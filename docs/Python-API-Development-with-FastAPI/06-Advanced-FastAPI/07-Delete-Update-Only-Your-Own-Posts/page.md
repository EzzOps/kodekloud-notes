...
Source: https://notes.kodekloud.com/docs/Python-API-Development-with-FastAPI/Advanced-FastAPI/Delete-Update-Only-Your-Own-Posts/page

This article explains how to enforce ownership checks for deleting and updating user posts in an application.
...
3. **Unauthorized Delete Attempt**\
   If the logged-in user (ID 23) tries to delete a post owned by another user (e.g., owner\_id 21), the endpoint correctly responds with a 403 Forbidden error.

4. **Successful Deletion and Update**\
   When the logged-in user attempts to delete or update a post they own (owner\_id 23), the operation is successful. For instance, deleting a post with ID 8 (where owner\_id is 23) returns a 204 status code. Similarly, updating the post modifies its content correctly.
