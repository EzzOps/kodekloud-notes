...
Source: https://notes.kodekloud.com/docs/Python-API-Development-with-FastAPI/Advanced-FastAPI/Sql-Joins/page
...
To correctly join the tables, specify the columns to match. Here, we link the post owner (posts.owner\_id) with the unique user identifier (users.id):
...
Next, let’s join the posts and votes tables. In the votes table, the key columns are post\_id and user\_id. By joining these tables on post\_id, you can determine which posts have received votes.
...
Then, perform a LEFT JOIN with the votes table by matching posts.id to votes.post\_id:
