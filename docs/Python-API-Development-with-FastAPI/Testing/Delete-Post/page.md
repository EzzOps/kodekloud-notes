...
Source: https://notes.kodekloud.com/docs/Python-API-Development-with-FastAPI/Testing/Delete-Post/page
...
Assuming that our posts data contains multiple posts and that the fourth post (index 3) is owned by a different user, the following test ensures that the authorized client (logged in as test\_user) receives a 403 Forbidden response when attempting to delete that post:
