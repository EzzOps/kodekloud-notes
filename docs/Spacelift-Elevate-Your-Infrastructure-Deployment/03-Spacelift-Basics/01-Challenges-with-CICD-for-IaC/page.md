...
> **lightbulb** Traditional CI/CD tools are typically designed with stateless applications in mind. This design often makes handling the state locks required by IaC tools more complex. While you can configure these tools to handle state locks, the increased complexity may hinder smooth operations.
...
> **triangle-alert** Always verify that your CI/CD pipeline is configured to handle state locks properly and conform to your organization’s access policies across environments. Misconfigurations can lead to deployment failures and potential state corruption.
